...
export default function Toolbox() {
  ...
  return <div className="toolbox-grid toolbox-v2">
    <section className="tool-card tool-wide calculator" id="vat-tool" data-testid="vat-tool" data-reveal>
      <div className="tool-head"><span>01</span><i aria-hidden="true">🧮</i><div><div className="tool-title-line"><h2>คำนวณภาษีมูลค่าเพิ่ม</h2><b>✎ กรอกและปรับค่าได้</b></div><p>บวกหรือถอด VAT พร้อมคัดลอกผลลัพธ์ไปใช้งานต่อ</p></div></div>
      <div className="tool-two-columns"><div className="tool-controls"><label><span className="editable-field-label">จำนวนเงิน <em>แก้ไขได้</em></span><input value={amount} onChange={(e) => setAmount(e.target.value)} inputMode="decimal" aria-label="จำนวนเงินสำหรับคำนวณ VAT" /></label><label><span className="editable-field-label">อัตรา VAT (%) <em>แก้ไขได้</em></span><input value={vatRate} onChange={(e) => setVatRate(e.target.value)} inputMode="decimal" aria-label="อัตรา VAT" /></label><div className="segmented" aria-label="เลือกรูปแบบ VAT"><button type="button" className={!inclusive ? "active" : ""} onClick={() => setInclusive(false)}>ยังไม่รวม VAT</button><button type="button" className={inclusive ? "active" : ""} onClick={() => setInclusive(true)}>รวม VAT แล้ว</button></div><small className="live-result-hint"><span>↻</span> เปลี่ยนค่าแล้วผลลัพธ์ด้านขวาจะคำนวณใหม่ทันที</small></div><dl className="vat-result"><div><dt>มูลค่าก่อนภาษี</dt><dd data-testid="vat-base">{money(vat.base)}</dd></div><div><dt>ภาษีมูลค่าเพิ่ม</dt><dd data-testid="vat-amount">{money(vat.vat)}</dd></div><div><dt>ยอดรวม</dt><dd data-testid="vat-total">{money(vat.total)}</dd></div><button type="button" className="tool-action" onClick={copyVat}>{copied ? "คัดลอกแล้ว ✓" : "คัดลอกผลลัพธ์"}</button></dl></div>
    ...
    <section className="tool-card" id="employee-cost-tool" data-testid="employee-cost-tool" data-reveal>
      <div className="tool-head"><span>02</span><i aria-hidden="true">👥</i><div><div className="tool-title-line"><h2>ประเมินต้นทุนพนักงาน</h2><b>✎ ปรับสมมติฐานได้</b></div><p>เห็นเงินเดือนและเงินสมทบฝั่งนายจ้างต่อเดือน</p></div></div>
      <div className="compact-fields"><label><span className="editable-field-label">เงินเดือนต่อคน <em>แก้ไขได้</em></span><input value={salary} onChange={(e) => setSalary(e.target.value)} inputMode="decimal" aria-label="เงินเดือนต่อคน" /></label><label><span className="editable-field-label">จำนวนพนักงาน <em>แก้ไขได้</em></span><input value={employees} onChange={(e) => setEmployees(e.target.value)} inputMode="numeric" aria-label="จำนวนพนักงาน" /></label><label><span className="editable-field-label">อัตราสมทบ (%) <em>แก้ไขได้</em></span><input value={ssoRate} onChange={(e) => setSsoRate(e.target.value)} inputMode="decimal" aria-label="อัตราเงินสมทบ" /></label><label><span className="editable-field-label">เพดานค่าจ้าง <em>แก้ไขได้</em></span><input value={ssoCap} onChange={(e) => setSsoCap(e.target.value)} inputMode="decimal" aria-label="เพดานค่าจ้างที่ใช้คำนวณ" /></label></div>
    ...
    <section className="tool-card checklist-card" id="checklist-tool" data-testid="checklist-tool" data-reveal>
      <div className="tool-head"><span>03</span><i aria-hidden="true">✅</i><div><div className="tool-title-line"><h2>เช็กลิสต์เปิดบริษัท</h2><b>✓ แตะเพื่อเพิ่มหรือลบ</b></div><p>ระบบจำสถานะไว้ในอุปกรณ์นี้โดยอัตโนมัติ</p></div></div>
    ...
    <section className="tool-card recommender" id="structure-tool" data-testid="structure-tool" data-reveal>
      <div className="tool-head"><span>04</span><i aria-hidden="true">🧭</i><div><div className="tool-title-line"><h2>แนะนำโครงสร้างกิจการ</h2><b>⌄ เลือกคำตอบได้</b></div><p>ตอบ 3 ข้อเพื่อดูตัวเลือกที่ควรนำไปเปรียบเทียบ</p></div></div>
      <label><span className="editable-field-label">จำนวนผู้ร่วมก่อตั้ง <em>เลือกได้</em></span><select value={owners} onChange={(e) => setOwners(e.target.value)} aria-label="จำนวนผู้ร่วมก่อตั้ง"><option value="one">เจ้าของคนเดียว</option><option value="many">มีผู้ร่วมก่อตั้งหลายคน</option></select></label>
      <label><span className="editable-field-label">ความต้องการแยกความรับผิด <em>เลือกได้</em></span><select value={risk} onChange={(e) => setRisk(e.target.value)} aria-label="ความต้องการแยกความรับผิด"><option value="normal">ความเสี่ยงยังไม่สูง</option><option value="separate">ต้องการแยกความรับผิดชัดเจน</option></select></label>
      <label><span className="editable-field-label">ลูกค้าหลัก <em>เลือกได้</em></span><select value={clients} onChange={(e) => setClients(e.target.value)} aria-label="ลูกค้าหลัก"><option value="consumer">บุคคลทั่วไป</option><option value="corporate">บริษัทหรือองค์กร</option></select></label>
    ...
    <section className="tool-card name-tool" id="name-tool" data-testid="name-tool" data-reveal>
      <div className="tool-head"><span>05</span><i aria-hidden="true">🔎</i><div><div className="tool-title-line"><h2>ตรวจความพร้อมชื่อบริษัท</h2><b>✎ พิมพ์ชื่อได้</b></div><p>ช่วยคัดข้อสังเกตก่อนตรวจและจองชื่อกับหน่วยงานจริง</p></div></div>
      <label><span className="editable-field-label">ชื่อบริษัทที่ต้องการ <em>พิมพ์หรือเปลี่ยนได้</em></span><input value={companyName} onChange={(e) => setCompanyName(e.target.value)} placeholder="ลองพิมพ์ เช่น ตั้งต้น โซลูชัน" aria-label="ชื่อบริษัทที่ต้องการตรวจ" /></label>
